		...
		<c:if test="${sessionScope.m.memberLevel eq 0 }">
			<button id="insert-btn" class="btn btn-default" onclick="location.href='/spaceInsertFrm.do'">새 공간 등록하기</button>
				...
				<div class="space-list">
					...
					<a id="info" href="/spaceView.do?spaceNo=${s.spaceNo }">>상세보기</a>
					<p>공간 : ${s.spaceName }</p>
					<p>용도 : ${s.spacePurpose }</p>
					<p>수용인원 :${s.maxPeople }</p>
					<p>가격 : ${s.price }</p>
						<c:if test="${sessionScope.m.memberLevel eq 0 }">
							<button id="del-btn" class="btn btn-default" onclick="location.href='/spaceDelete.do?spaceNo=${s.spaceNo}'">삭제하기</button>
							<button id="update-btn" class="btn btn-default" onclick="location.href='/spaceUpdate.do?spaceNo=${s.spaceNo}'">수정하기</button>
						</c:if>
		</form>				
	</div>
			</c:forEach>
